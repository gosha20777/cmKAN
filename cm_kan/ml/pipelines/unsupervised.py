...
class UnsupervisedPipeline(L.LightningModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        Logger.info(
            "Initialized model weights with [bold green]Unsupervised[/bold green] pipeline."
        )
        if self.pretrained:
            Logger.info("Model is in [bold green]CycleGAN training[/bold green] mode.")
        else:
            Logger.info(
                "Model is in [bold green]Generator pre-training[/bold green] mode."
            )
        ...
        if not self.pretrained:
            if self.optimizer_type == "adam":
                optimizer = optim.Adam(
                    itertools.chain(
                        self.model.gen_ab.parameters(), self.model.gen_ba.parameters()
                    ),
                    lr=self.lr,
                    weight_decay=self.weight_decay,
                )
            ...
            if self.optimizer_type == "adam":
                ...
                optD = optim.Adam(
                    itertools.chain(
                        self.model.dis_a.parameters(), self.model.dis_b.parameters()
                    ),
                    lr=2e-4,
                    betas=(0.5, 0.999),
                )
    ...
    def generator_training_step(self, imgA, imgB):
        ...
        identityLoss = self._identity_loss(sameA, imgA) + self._identity_loss(
            sameB, imgB
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if not self.pretrained:
            loss = self.generator_pretaining_step(
                img_ab_recolorized, img_a, img_ba_recolorized, img_b
            )
            return {"loss": loss}
        else:
            ...
            self._set_requires_grad(
                [self.model.dis_a, self.model.dis_b], requires_grad=True
            )
