...
class ColorMatching:
    def __init__(self):
        ...
        self.detector = cv2.xfeatures2d.SURF.create(
            hessianThreshold=self.surf_hessian_trashold
        )
    ...
    def match_features(self, src_img, ref_img, scale=None):
        ...
        if scale is not None:
            src_img = cv2.resize(
                src_img, (int(src_img.shape[1] / scale), int(src_img.shape[0] / scale))
            )
            ref_img = cv2.resize(
                ref_img, (int(ref_img.shape[1] / scale), int(ref_img.shape[0] / scale))
            )
        ...
        if self.cross_check:
            ...
            matches = self.matcher.knnMatch(
                src_descriptors, ref_descriptors, k=2
            )  # , self.surf_knn_match)
